...
| [`archive_sources.py`](archive_sources.py) | `archived_url` + `archived_at` fields in `../data/*.json` | Every source URL in the data files; the Wayback Machine availability API | After adding or repointing any citation. `--dry-run` reports coverage; `--save` asks Wayback to capture URLs with no snapshot (slow, rate-limited). The archived copy never replaces the citation — the publisher's URL stays canonical. |
