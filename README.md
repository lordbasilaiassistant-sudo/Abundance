...
- **[`llms.txt`](llms.txt)** — a machine-readable index of every page and dataset here, for agents that want the data rather than the prose. `robots.txt` welcomes AI crawlers explicitly: this site exists to be cited.
...
```
Abundance/
├── index.html              # editorial homepage (generated by scripts/build_landing.py)
├── essay.html              # the full arithmetic essay (commented sections)
├── letter.html             # open letter to whoever can act — the moral close
├── course.html             # structured reading path through the material
├── countries.html          # per-country drill-down (reads data/countries.json)
├── case-studies.html       # country-scale precedents
├── datacenter-water.html   # design note: waste heat → drinking water
...
│   ├── editorial.css       # shared reading theme (essay, letter, course, …)
...
├── lang/{es,fr,pt,ar,zh,hi}/  # translations — all have letter.html; es has index.html
│
├── papers/                 # deep notes on specific scholarly works
├── scripts/                # regeneration utilities (Python) + landing.js
│   └── check.py            # repo self-check — run before every PR
├── transcripts/            # raw research material (auto-VTT downloads)
├── assets/planet.svg       # original procedural illustration (generated)
│
...
├── robots.txt, sitemap.xml # crawler surface (AI crawlers explicitly welcome)
└── README.md
```

Before opening a pull request, run the self-check:
...
For "where do I add X?" see [`AGENTS.md`](AGENTS.md) (repo map, invariants, task recipes) and [`CONTRIBUTING.md`](CONTRIBUTING.md) (the evidence standard).
